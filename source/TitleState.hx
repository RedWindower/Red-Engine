	...
	function startIntro()
	{
		...
		switch(easterEgg.toUpperCase())
		{
			...
			case 'SHADOW':
				...
				case 'WINDOWS':
				gfDance.frames = Paths.getSparrowAtlas('WINDOWSBump');
				gfDance.animation.addByPrefix('danceLeft', 'danceLeft', 24);
				gfDance.animation.addByPrefix('danceRight', 'danceRight', 24);
			case 'RIVER':
	...
	override function beatHit()
	{
		...
		if(!closedState) {
			...
			switch (sickBeats)
			{
				...
				case 2:
					#if PSYCH_WATERMARKS
					createCoolText(['Red Engine 1.4.1 by'], 40);
					#else
		...
		if (!skippedIntro)
		{
			if (playJingle) //Ignore deez
			{
				...
				switch(easteregg)
				{
					...
					case 'WINDOWS':
					    sound = FlxG.sound.play(Paths.sound('JingleWindows'));
